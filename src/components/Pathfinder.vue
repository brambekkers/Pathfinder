<template>
    <div id="pathfinder">
        <div id="grid" @mousedown="startDrag" @mousemove="doDrag">
            <div v-for="(row, index) of grid" class="row" :key="`row${index}`" :style="{ height: cellSize + 'px' }">
...
export default {
    ...
    computed: {
        ...mapGetters(["grid", "cellSize", "addItem"])
    },
    methods: {
        createNewGrid(e) {
            this.calcCellAmount();
        ...
        calcCellAmount() {
            ...
            this.cellAmount.height = Math.floor(div.offsetHeight / this.cellSize);
        },
        createGrid() {
            const grid = [];
        ...
        doDrag(event) {
            if (this.dragging && this.addItem === "wall") {
                const newX = Math.floor(event.clientX / this.cellSize) - 1;
                const newY = Math.floor((event.clientY - this.cellSize / 2) / this.cellSize) - 5;

                if (newX != this.x || newY != this.y) {
                    this.x = newX;
                    this.y = newY;

                    if (this.grid[newY] && this.grid[newY][newX] && !this.grid[newY][newX].disableInput) {
